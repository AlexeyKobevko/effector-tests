...
export const seoTags = (config: BuildTagsParams) => {
  ...
  if (config.twitter) {
    ...
    if (config.twitter.title) {
      tagsToRender.push(
        <meta key="twitter:title" name="twitter:title" content={config.twitter.title} />
      );
    }

    if (config.twitter.images) {
      config.twitter.images.forEach((image, index) =>
        tagsToRender.push(
          <meta key={`twitter:image${index}` as string} name="twitter:image" content={image} />
        )
      );
    }

    if (config.twitter.imagesSrc) {
      config.twitter.imagesSrc.forEach((image, index) =>
        tagsToRender.push(
          <meta
            key={`twitter:image:src${index}` as string}
  ...
  if (config.facebook) {
    if (config.facebook.appId) {
      tagsToRender.push(
        <meta key="fb:app_id" property="fb:app_id" content={config.facebook.appId} />
      );
    }
  }
  ...
  if (config.openGraph) {
    if (config.openGraph.url || config.canonical) {
      tagsToRender.push(
        <meta key="og:url" property="og:url" content={config.openGraph.url || config.canonical} />
      );
    }
      ...
      if (type === 'profile' && config.openGraph.profile) {
        ...
        if (config.openGraph.book.authors && config.openGraph.book.authors.length) {
          config.openGraph.book.authors.forEach((author, index) => {
            tagsToRender.push(
              <meta
                key={`book:author:0${index}` as string}
        ...
        if (config.openGraph.book.isbn) {
          tagsToRender.push(
            <meta key="book:isbn" property="book:isbn" content={config.openGraph.book.isbn} />
          );
        }
        ...
        if (config.openGraph.book.tags && config.openGraph.book.tags.length) {
          config.openGraph.book.tags.forEach((tag, index) => {
            tagsToRender.push(
              <meta key={`book:tag:0${index}` as string} property="book:tag" content={tag} />
            );
          });
        }
        ...
        if (config.openGraph.article.publisher) {
          tagsToRender.push(
            <meta
              key="article:publisher"
              property="article:publisher"
              content={config.openGraph.article.publisher}
            />
          );
        }
        ...
        if (config.openGraph.article.authors && config.openGraph.article.authors.length) {
          config.openGraph.article.authors.forEach((author, index) => {
            tagsToRender.push(
              <meta
                key={`article:author:0${index}` as string}
        ...
        if (config.openGraph.article.tags && config.openGraph.article.tags.length) {
          config.openGraph.article.tags.forEach((tag, index) => {
            tagsToRender.push(
              <meta key={`article:tag:0${index}` as string} property="article:tag" content={tag} />
            );
          });
        }
      } else if (
        ...
        if (config.openGraph.video.actors && config.openGraph.video.actors.length) {
          config.openGraph.video.actors.forEach((actor, index) => {
            if (actor.profile) {
              tagsToRender.push(
                <meta
                  key={`video:actor:0${index}` as string}
            ...
            if (actor.role) {
              tagsToRender.push(
                <meta
                  key={`video:actor:role:0${index}` as string}
        ...
        if (config.openGraph.video.directors && config.openGraph.video.directors.length) {
          config.openGraph.video.directors.forEach((director, index) => {
            tagsToRender.push(
              <meta
                key={`video:director:0${index}` as string}
        ...
        if (config.openGraph.video.writers && config.openGraph.video.writers.length) {
          config.openGraph.video.writers.forEach((writer, index) => {
            tagsToRender.push(
              <meta
                key={`video:writer:0${index}` as string}
        ...
        if (config.openGraph.video.tags && config.openGraph.video.tags.length) {
          config.openGraph.video.tags.forEach((tag, index) => {
            tagsToRender.push(
              <meta key={`video:tag:0${index}` as string} property="video:tag" content={tag} />
            );
          });
        }
    ...
    if (config.openGraph.images && config.openGraph.images.length) {
      config.openGraph.images.forEach((image, index) => {
        tagsToRender.push(
          <meta key={`og:image:0${index}` as string} property="og:image" content={image.url} />
        );

        if (image.alt) {
          tagsToRender.push(
            <meta
              key={`og:image:alt0${index}` as string}
        ...
        if (image.width) {
          tagsToRender.push(
            <meta
              key={`og:image:width0${index}` as string}
        ...
        if (image.height) {
          tagsToRender.push(
            <meta
              key={`og:image:height${index}` as string}
    ...
    if (config.openGraph.videos && config.openGraph.videos.length) {
      config.openGraph.videos.forEach((video, index) => {
        tagsToRender.push(
          <meta key={`og:video:0${index}` as string} property="og:video" content={video.url} />
        );

        if (video.alt) {
          tagsToRender.push(
            <meta
              key={`og:video:alt0${index}` as string}
        ...
        if (video.width) {
          tagsToRender.push(
            <meta
              key={`og:video:width0${index}` as string}
        ...
        if (video.height) {
          tagsToRender.push(
            <meta
              key={`og:video:height${index}` as string}
  ...
  if (config.additionalMetaTags && config.additionalMetaTags.length > 0) {
    config.additionalMetaTags.forEach((tag) => {
      tagsToRender.push(<meta key={tag.content} {...tag} />);
    });
  }

  if (config.additionalLinkTags && config.additionalLinkTags.length > 0) {
    config.additionalLinkTags.forEach((tag) => {
      tagsToRender.push(<link key={tag.href} {...tag} />);
    });
  }

  if (config.googleSite && config.googleSite.length > 0) {
    config.googleSite.forEach((item) => {
      tagsToRender.push(<meta key={item} name="google-site-verification" content={item} />);
    });
  }

  if (config.style) {
    tagsToRender.push(<link key="stylesheet" rel="stylesheet" href={config.style} />);
  }

  if (config.bodyClass) {
    tagsToRender.push(<body key={config.bodyClass} className={config.bodyClass} />);
  }
